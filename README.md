...
| 기능 | 설명 |
|---|---|
| 심플 당뇨 예측 | 나이, 성별, BMI, 허리둘레, 공복 혈당(선택) 기반으로 빠르게 위험도 확인 |
| 상세 당뇨 예측 | 입력값 직접 입력 + 선택형 보강 질문(F1 가족력, F2 고혈압/혈압약)으로 정밀도 보강 |
| ML 예측 엔진 자동 선택 | 입력 항목(예: 허리둘레·공복혈당) 구성에 맞춰 머신러닝 예측 엔진을 자동 적용하고, 서버 입력 검증을 함께 수행 |
| 결과 시각화 | 위험 확률, 판정 라벨, 차트(내 수치 vs 정상 범위) 제공 |
| 병원 검색 | 저장한 주소 좌표 기준 주변 병원 목록 조회 (공공데이터 API) |
| 길찾기 연동 | 카카오맵/네이버지도/티맵/Apple Maps 등 설치된 지도 앱으로 즉시 이동 |
| 당뇨 건강정보 | 진단 참고 기준, 주요 증상, 예방·관리 수칙, 응급 대응 정보를 카드형으로 제공 |
| 설정 기능 | 라이트/다크 테마 전환, API 서버 주소 설정(디버그), 주소/좌표 로컬 저장 |
